<template>
	<div class="layout-navbars-breadcrumb-user pr15" :style="{ flex: layoutUserFlexNum }">
		<!-- 组件大小切换按钮 - 已隐藏 -->
		<el-dropdown v-show="false" :show-timeout="70" :hide-timeout="50" trigger="click"
			...
			<template #dropdown>
				<el-dropdown-menu>
					<el-dropdown-item command="large"
							:disabled="state.disabledSize === 'large'">{{ $t('message.user.dropdownLarge') }}</el-dropdown-item>
					<el-dropdown-item command="default"
							:disabled="state.disabledSize === 'default'">{{ $t('message.user.dropdownDefault') }}</el-dropdown-item>
					<el-dropdown-item command="small"
							:disabled="state.disabledSize === 'small'">{{ $t('message.user.dropdownSmall') }}</el-dropdown-item>
				</el-dropdown-menu>
			</template>
		</el-dropdown>
		<!-- 语言切换按钮 - 已隐藏 -->
		...
		<!-- 搜索按钮 - 已隐藏 -->
		<div class="layout-navbars-breadcrumb-user-icon" @click="onSearchClick" style="display: none">
		...
		<!-- 全屏按钮 - 已隐藏 -->
		<div v-show="false" class="layout-navbars-breadcrumb-user-icon mr10" @click="onScreenfullClick">
		...
		<div></div>
		<el-dropdown :show-timeout="70" :hide-timeout="50" @command="onHandleCommandClick" style="margin-left:10px">
...
// 引入组件
const UnreadNoticeBadge = defineAsyncComponent(() => import('/@/views/plugins/bs-notice/components/UnreadNoticeBadge/index.vue'));
...
onMounted(() => {
	...
	noticeRefreshTimer = setInterval(fetchNoticeUnreadCount, 30000);
});

//消息中心的未读数量
import { messageCenterStore } from '/@/stores/messageCenter';
import { getBaseURL } from '/@/utils/baseUrl';
import { getUnreadCount as getNoticeUnreadCount } from '/@/views/plugins/bs-notice/api';
const messageCenter = messageCenterStore();

// bs_notice 未读数量（独立于 SSE）
let noticeRefreshTimer: any = null;
const fetchNoticeUnreadCount = async () => {
	try {
		const res = await getNoticeUnreadCount();
		if (res.code === 2000) {
			messageCenter.setUnread(res.data?.count || 0);
		}
	} catch (error) {
		console.error('获取通知未读数量失败', error);
	}
};
...
const getMessageCenterCount = () => {
	...
	eventSource.onmessage = function (event) {
		...
		// messageCenter.setUnread(+event.data); // 旧消息中心，已改用 bs_notice 未读数
	};
