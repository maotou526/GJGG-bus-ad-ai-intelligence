<template>
	<div class="h100" v-show="!isTagsViewCurrenFull">
		<el-aside class="layout-aside" :class="setCollapseStyle">
			...
			<!-- 底部菜单收起/展开按钮 -->
			<div class="layout-aside-toggle" @click="onToggleMenuCollapse">
				<div class="toggle-icon">
					<el-icon v-if="themeConfig.isCollapse">
						<DArrowRight />
					</el-icon>
					<el-icon v-else>
						<DArrowLeft />
...
// const hideMenuPaths = ['/system', '/generalConfig', '/log', '/celeryManage', '/workflow', '/osm', '/od', '/bst', '/tx', '/thirdpartydata', '/passengerflowanalysis', '/networkPassengerFlowAnalysis','/PassengerFlowAnalysis/StationPassengerFlowAnalysis'];
const hideMenuPaths = [];
 ...
 * @returns 过滤后的菜单列表
 */
const filterMenuList = (list: RouteItem[] = []): RouteItem[] => {
	return list
		.filter((item: RouteItem) => !hideMenuPaths.some((path) => item.path?.startsWith(path)))
		.map((item: RouteItem) => {
			// 如果有子菜单，递归过滤
			if (item.children && item.children.length > 0) {
...
const setFilterRoutes = (path='') => {
	...
	state.menuList = filterMenuList(filterRoutesFun(routesList.value[routesIndex.value].children || [routesList.value[routesIndex.value]]));
	} else { 
		state.menuList = filterMenuList(filterRoutesFun(routesList.value));
