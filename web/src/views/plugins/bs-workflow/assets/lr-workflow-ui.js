/*
 * 版 本 Learun-ADMS-Core 力软敏捷开发框架(http://www.learun.cn)
...
(function ($, learun) {
    ...
    $.lrworkflow = {
        ...
        addNode: function ($workArea, dfop, node, isold) {
            ...
            if (node.state != undefined && (node.type == 'startround' || node.type == 'auditornode' || node.type == 'stepnode' || node.type == 'confluencenode' || node.type == 'childwfnode')) {
            ...
            if (!isold) {
                switch (node.type) {
                    case 'startround':
                        node.wfForms = [];
                        node.notice = "";
                        node.isNext = "2";      // 下一节点审核人不允许手动指定
                        node.isTitle = "2";     // 是否允许自定义标题否
                        break;
                    case 'stepnode':// 普通节点
                        node.auditors = [];
                        node.wfForms = [];
                        node.btnList = [{ id: '1', name: '同意', code: 'agree', file: '1', next: '1' }, { id: '2', name: '不同意', code: 'disagree', file: '1', next: '1' }];
                        break;
                    case 'auditornode':// 审阅节点
                        node.auditors = [];
                        node.wfForms = [];
                        break;
        ...
        initNodeEvent: function ($workArea) {
            ...
            $workArea.delegate(".lr-workflow-node", "dblclick", { $workArea: $workArea }, function (e) {
                ...
                var $node = $(this);
                var nodeData = $node[0].wfdata;
                dfop.openNode(nodeData, dfop.node);
            });
            if (!dfop.isPreview) {
                //绑定点击事件
                ...
                $workArea.mousemove(function (e) {
                    ...
                    // 检查临时线条是否存在
                    if (!line || !line.childNodes || line.childNodes.length < 2) return;
        ...
        initLineEvent: function ($workArea) {
            ...
            if (!dfop.isPreview) {
                ...
                $workArea.delegate('g', "dblclick", { $workArea: $workArea }, function (e) {
                    ...
                    var nodeData = null;
                    $.each(dfop.node, function (_index, _node) {
                        if (_node.id == lineData.from) {
                            nodeData = _node;
                            return false;
                        }
                    });
                    dfop.openLine(lineData, nodeData);
                });
            }
        },
        // 获取线条数据
        ...
        addLine: function (dfop, line) {
            ...
            var _isReturn = false;
            $.each(dfop.node, function (_index, _node) {
                if (_node.id == line.from) {
                    if (_node.type == 'endround' || _node.type == 'auditornode') {
                        _isReturn = true;
                    }
                    return false;
                }
            ...
            if (line.type && line.type != "sl") {
                ...
                $line = $.lrworkflow.drawPoly(line.color, line.id, res.start, res.m1, res.m2, res.end, line.mark);
            }
            else {
                line.type = "sl";//默认为直线
                $line = $.lrworkflow.drawLine(line.color, line.id, sxy, exy, line.mark);
            }
            var $draw = $('#' + dfop.id).find('svg');
        ...
        setLineType: function (id, newType) {
            ...
            if (newType != "sl") {
                ...
                $line.remove();
                $line = $.lrworkflow.drawLine(lineData.color, lineData.id, sxy, exy, lineData.mark);
                var $draw = $('#' + dfop.id).find('svg');
                $($line)[0].dfop = dfop;
                $draw.append($line);
                var lineData = $.lrworkflow.getLine(dfop, id);
        ...
        setLineM: function (id, M, noStack) {
            ...
            $('#' + id).remove();
            var $line = $.lrworkflow.drawPoly(lineData.color, id, ps.start, ps.m1, ps.m2, ps.end, lineData.marked || dfop.focusId == id);
            var $draw = $('#' + dfop.id).find('svg');
    ...
    $.fn.lrworkflow = function (op) {
        ...
        var dfop = {
            openNode: function () { },
            openLine: function () { },
            toolBtns: ["startround", "endround", "stepnode", "confluencenode", "conditionnode", "auditornode","childwfnode"],//"childwfnode"
            nodeRemarks: {
                cursor: "选择指针",
                direct: "步骤连线",
                startround: "开始节点",
