<template>
	...
	<el-dialog v-model="dialogVisible" title="数据权限配置" :width="selectBtn.data_range === 5 ? '1000px' : '400px'" :close-on-click-modal="false" :before-close="handleDialogClose">
		<div class="pc-dialog">
...
// 当前模型名称（根据菜单动态获取，默认为空表示通用规则）
// TODO: 可以根据当前选择的菜单自动识别对应的模型
...
const handlePermissionRangeChange = async (val: number) => {
	...
	// 清空/初始化自定义规则
	if (val !== 5) {
		selectBtn.value.custom_rules = null;
	} else if (!selectBtn.value.custom_rules) {
		selectBtn.value.custom_rules = { logic: '', rules: [] };
	}
};
/**
 * 格式化按钮数据范围
...
const formatDataRange = computed(() => {
	return function (datarange: number, dept: Array<number>) {
		...
		if (datarange === default_selectBtn.value.data_range) {
			// 判断选择的部门是否一致（data_range=4）或自定义规则是否一致（data_range=5）
			if (datarange !== 4 && datarange !== 5 || 
				(datarange === 4 && JSON.stringify(dept) === JSON.stringify(default_selectBtn.value.dept))) {
				
				return "默认接口权限"
			}
		}
		// datarange === 4 或 datarange === 5 选择的部门/规则不一致返回datarangeitem.label
...
const handleDialogConfirm = async () => {
	// 获取权限记录ID（兼容两种字段名）
	const permId = selectBtn.value.role_menu_btn_perm_id || selectBtn.value.menu_btn_pre_id;
	
	// 检查是否有权限记录ID
	if (!permId || permId === 0) {
		ElMessage({ 
			message: '请先勾选该按钮后再配置数据权限', 
			type: 'warning' 
		});
		return;
	}
