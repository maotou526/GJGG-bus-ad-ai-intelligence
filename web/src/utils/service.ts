...
function createService() {
	...
	service.interceptors.response.use(
		(response) => {
			// 设置 flag，默认判断是否是生产环境
			const shouldIncludeUrl = !import.meta.env.PROD;
			// const shouldIncludeUrl = false;
			...
			if (code === undefined) {
				...
				const errorMsg = shouldIncludeUrl 
					? `非标准返回：${dataAxios}， ${response.config.url}`
					: `非标准返回：${dataAxios}`;
				errorCreate(errorMsg, false);
				return dataAxios;
			} else {
				...
				switch (code) {
					case 400:
						// Local.clear();
						// Session.clear();
						const errorMsg400 = shouldIncludeUrl 
							? `${dataAxios.msg}: ${response.config.url}`
							: dataAxios.msg;
						errorCreate(errorMsg400);
					...
					case 401:
						...
						const errorMsg401 = shouldIncludeUrl 
							? `${dataAxios.msg}: ${response.config.url}`
							: dataAxios.msg;
						errorCreate(errorMsg401);
						break;
					case 2000:
						// @ts-ignore
						...
						return dataAxios;
					case 4000:
						const errorMsg4000 = shouldIncludeUrl 
							? `${dataAxios.msg}: ${response.config.url}`
							: dataAxios.msg;
						errorCreate(errorMsg4000);
						break;
					default:
						// 不是正确的 code
						const errorMsgDefault = shouldIncludeUrl 
							? `${dataAxios.msg}: ${response.config.url}`
							: dataAxios.msg;
						errorCreate(errorMsgDefault);
						break;
				}
