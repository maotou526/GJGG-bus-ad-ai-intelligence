...
class DataLevelPermissionsFilter(BaseFilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if request.user.is_superuser == 0:
            # 收集标准权限的 Q 对象
            q_obj = self._build_standard_permission_q(request, queryset, api, method)
            
            if q_obj:
                # 初始化 Q 对象列表
                if not hasattr(request, '_data_permission_q_list'):
                    request._data_permission_q_list = []
                request._data_permission_q_list.append(q_obj)
                import logging
                logger = logging.getLogger(__name__)
                logger.debug(f"收集标准权限 Q: 用户={request.user.username}")
    ...
    def _build_standard_permission_q(self, request, queryset, api, method):
        """
        构建标准权限的 Q 对象（提取原有逻辑）
        
        Returns:
            Q 对象，如果不需要过滤则返回 None
        """
        ...
        if not user_dept_id:
            return Q(id__in=[])  # 返回空条件

        # 1. 判断过滤的数据是否有创建人所在部门 "dept_belong_id" 字段
        if not getattr(queryset.model, "dept_belong_id", None):
            return None  # 模型没有 dept_belong_id，返回 None（不应用过滤）

        # 2. 如果用户没有关联角色则返回本部门数据
        if not hasattr(request.user, "role"):
            return Q(dept_belong_id=user_dept_id)
        ...
        for ele in role_permission_list:
            ...
            if ele.get("data_range") == 3:
                return None  # 全部数据权限，不应用过滤
            dataScope_list.append(ele.get("data_range"))
        dataScope_list = list(set(dataScope_list))

        # 4. 只为仅本人数据权限时只返回过滤本人数据，并且部门为自己本部门(考虑到用户会变部门，只能看当前用户所在的部门数据)
        if 0 in dataScope_list:
            return Q(creator=request.user, dept_belong_id=user_dept_id)
        ...
        if queryset.model._meta.model_name == 'dept':
            return Q(id__in=list(set(dept_list)))
        return Q(dept_belong_id__in=list(set(dept_list)))
