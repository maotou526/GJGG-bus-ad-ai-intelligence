...
def get_and_validate_date_param(input_str, required=True, date_format='YYYY-MM-DD'):
    """
    验证并转换日期字符串
    ...
    Args:
        ...
        date_format: 日期格式，支持 'YYYY-MM-DD' 或 'YYYY-MM'，默认为 'YYYY-MM-DD'
        
    Returns:
        tuple: (date_obj, error_response)
        - 如果成功：返回 (date对象, None)
        - 如果失败：返回 (None, ErrorResponse对象)
    """
    from datetime import datetime
    from dvadmin.utils.json_response import ErrorResponse
    
    if not input_str and required:
        format_hint = "YYYY-MM-DD" if date_format == 'YYYY-MM-DD' else "YYYY-MM"
        return None, ErrorResponse(msg=f"缺少必要参数 (格式: {format_hint})")
    ...
    try:
        if date_format == 'YYYY-MM-DD':
            ...
            # 对于 YYYY-MM 格式，转换为该月的第一天
            date_obj = datetime.strptime(input_str, '%Y-%m').date()
            return date_obj, None
        elif date_format == 'YYYY':
            date_obj = datetime.strptime(input_str, '%Y').date()
            return date_obj, None
        else:
            return None, ErrorResponse(msg=f"不支持的日期格式: {date_format}，仅支持 YYYY-MM-DD 或 YYYY-MM")
    except ValueError:
        format_hint = "YYYY-MM-DD" if date_format == 'YYYY-MM-DD' else "YYYY-MM"
        return None, ErrorResponse(msg=f"日期格式错误，请使用 {format_hint} 格式")