...
class SoftDeleteModel(models.Model):
    ...
    一旦继承,就将开启软删除
    """
    is_deleted = models.BooleanField(verbose_name="是否软删除", help_text='是否软删除', default=False, db_index=True, db_comment="是否软删除")
...
class CoreModelManager(models.Manager):
    def get_queryset(self):
        # 检查模型是否继承 FlowBaseModel（通过 IS_FLOW 属性判断）
        is_flow_model = getattr(self.model, 'IS_FLOW', False)
        is_deleted = getattr(self.model, 'is_soft_delete', False)
        queryset = super().get_queryset()
        
        # 只有继承 FlowBaseModel 的模型才过滤 flow_work_status
        if is_flow_model:
    ...
    def create(self, request: Request=None, **kwargs):
        ...
        if request:
            ...
            data["modifier"] = request_user  # 改为用户对象
...
class CoreModel(models.Model):
    ...
    id = models.CharField(
        ...
        editable=False,
        verbose_name="ID",
        help_text="UUID主键（字符串格式）",
        db_comment="UUID主键（字符串格式）"
    )
    description = models.CharField(max_length=255, verbose_name="描述", null=True, blank=True, help_text="描述", db_comment="描述")
    ...
    dept_belong_id = models.CharField(max_length=255, help_text="数据归属部门", null=True, blank=True,
                                      verbose_name="数据归属部门", db_comment="数据归属部门")
