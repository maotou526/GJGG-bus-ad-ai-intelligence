...
Version: 1.0
Autor: 王晨
Date: 2025-10-27 13:30:17
LastEditors: 王晨
LastEditTime: 2025-10-27 14:16:57
'''
"""
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    data_types = {
        ...
        'CharField': 'VARCHAR(%(max_length)s char)',
        'CommaSeparatedIntegerField': 'VARCHAR(%(max_length)s char)',
        'DateField': 'DATE',
        'DateTimeField': 'TIMESTAMP',
        'DecimalField': 'NUMBER(%(max_digits)s, %(decimal_places)s)',
        'DurationField': 'INTERVAL DAY(9) TO SECOND(6)',
        'FileField': 'VARCHAR(%(max_length)s char)',
        'FilePathField': 'VARCHAR(%(max_length)s char)',
        ...
        'SlugField': 'VARCHAR(%(max_length)s char)',
    ...
    def _connect_string(self):
        ...
        user = settings_dict['OPTIONS'].get('user', str(settings_dict['USER']).strip())
        passwd = settings_dict['OPTIONS'].get('passwd', str(settings_dict['PASSWORD']).strip())
        host = settings_dict['OPTIONS'].get('host', str(settings_dict['HOST']).strip())
        ...
        with self.cursor() as cursor:
            ...
            if schema_name:
                try:
                    ...
                    # 如果设置失败，记录警告但不中断
                    import warnings
                    warnings.warn(f"无法设置 schema {schema_name}: {e}")
...
class CursorWrapper(object):
    ...
    def execute(self, query, args=None):
        try:
            ...
            try:
                #query = self._check_placeholder(query)
                if args is None:
            ...
            #except UnicodeEncodeError as e:
                #i = 0
                    ...
                    #while i < len(args):
                            ...
                            #args[i] = args[i].encode('utf8)')
                        #i = i+1
                    
                #return self.cursor.execute(query.encode('utf8'), args)
