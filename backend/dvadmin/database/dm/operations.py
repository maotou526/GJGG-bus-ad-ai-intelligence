...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    compiler_module = "dvadmin.database.dm.compiler"  # 使用完整路径
...
"""    
    ...
    def _convert_field_to_tz(self, field_name, tzname):
        ...
        if self.connection.timezone_name != tzname:
            ...
            #return "CAST((FROM_TZ(%s, '%s')) AS TIMESTAMP)" % (
                #field_name,
                #self.connection.timezone_name
            #)            
        return field_name      
    ...
    def field_cast_sql(self, db_type, internal_type):
        ...
        #if db_type and db_type.startswith('TEXT') and internal_type == 'JSONField':
            #return "DBMS_LOB.SUBSTR(%s)"
        else:
            return "%s"
        ...
        with self.connection.cursor() as cursor:
            if recursive:
                cursor.execute("""
                    ...
                    #select r_tab_name, r_cons_name from cons_view start with p_tab_name = ? connect by prior r_tab_name=p_tab_name;
                #""",
                   (table_name.upper(),)
                )
            else:
    ...
    #added from Django version 3.1.7
