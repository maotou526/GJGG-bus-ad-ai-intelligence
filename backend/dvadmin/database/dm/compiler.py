...
from django.db.models.fields import BinaryField, TextField
...
from django.db.models.expressions import OrderBy, Exists, Col
    ...
    def compile(self, node, select_format=False):
        ...
        try:
            if isinstance(node, Col):
                sql, params = super(SQLCompiler, self).compile(node)
            elif isinstance(node, KeyTransform):
                # Oracle/DM can extract json field directly after 12.2, without JSON_QUERY
            ...
            """
            FullResultSet 表示 WHERE 子句为空（匹配所有记录）
            返回空的 SQL 和参数
            """
            return '', []
            ...
            if self.query.subquery and extra_select:
                ...
                for index, (select, _, alias) in enumerate(self.select, start=1):
                    ...
                    if alias:
                        ...
                        select_clone = select.relabeled_clone({select.alias: self.connection.ops.quote_name('subquery')})
