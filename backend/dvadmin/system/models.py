...
class Role(CoreModel, FlowBaseModel):
    name = models.CharField(max_length=64, verbose_name="角色名称", help_text="角色名称", null=True, blank=True)
    key = models.CharField(max_length=64, unique=True, verbose_name="权限字符", help_text="权限字符", null=True, blank=True)
    sort = models.IntegerField(default=1, verbose_name="角色顺序", help_text="角色顺序", null=True, blank=True)
    status = models.BooleanField(default=True, verbose_name="角色状态", help_text="角色状态", null=True, blank=True)
...
class Users(CoreModel, AbstractUser):
    username = models.CharField(max_length=150, unique=True, db_index=True, verbose_name="用户账号",
                                help_text="用户账号", null=True, blank=True)
    email = models.EmailField(max_length=255, verbose_name="邮箱", null=True, blank=True, help_text="邮箱")
    mobile = models.CharField(max_length=255, verbose_name="电话", null=True, blank=True, help_text="电话")
    avatar = models.CharField(max_length=255, verbose_name="头像", null=True, blank=True, help_text="头像")
    # 重写AbstractUser的first_name和last_name字段，允许NULL值
    first_name = models.CharField(max_length=150, blank=True, null=True, verbose_name="名字", help_text="名字")
    last_name = models.CharField(max_length=150, blank=True, null=True, verbose_name="姓氏", help_text="姓氏")
    name = models.CharField(max_length=40, verbose_name="姓名", help_text="姓名", null=True, blank=True)
    GENDER_CHOICES = (
        (0, "未知"),
        (1, "男"),
        (2, "女"),
    ...
    login_error_count = models.IntegerField(default=0, verbose_name="登录错误次数", help_text="登录错误次数", null=True, blank=True)
    pwd_change_count = models.IntegerField(default=0, verbose_name="密码修改次数", help_text="密码修改次数", null=True, blank=True)
    # 数据归属公司（与dept_belong_id一致的结构，用于公司级数据域）
    company_belong_id = models.CharField(max_length=255, null=True, blank=True, verbose_name="数据归属公司", help_text="数据归属公司")
    # 数据归属代理商
...
class Post(CoreModel):
    name = models.CharField(max_length=64, verbose_name="岗位名称", help_text="岗位名称", null=True, blank=True)
    code = models.CharField(max_length=32, verbose_name="岗位编码", help_text="岗位编码", null=True, blank=True)
    sort = models.IntegerField(default=1, verbose_name="岗位顺序", help_text="岗位顺序", null=True, blank=True)
    STATUS_CHOICES = (
        (0, "离职"),
        (1, "在职"),
    )
    status = models.IntegerField(choices=STATUS_CHOICES, default=1, verbose_name="岗位状态", help_text="岗位状态", null=True, blank=True)
...
class Dept(CoreModel):
    name = models.CharField(max_length=64, verbose_name="部门名称", help_text="部门名称", null=True, blank=True)
    key = models.CharField(max_length=64, unique=True, null=True, blank=True, verbose_name="关联字符", help_text="关联字符")
    sort = models.IntegerField(default=1, verbose_name="显示排序", help_text="显示排序", null=True, blank=True)
...
class Menu(CoreModel):
    ...
    name = models.CharField(max_length=64, verbose_name="菜单名称", help_text="菜单名称", null=True, blank=True)
    sort = models.IntegerField(default=1, verbose_name="显示排序", null=True, blank=True, help_text="显示排序")
    ISLINK_CHOICES = (
        (0, "否"),
        (1, "是"),
    )
    is_link = models.BooleanField(default=False, verbose_name="是否外链", help_text="是否外链", null=True, blank=True)
    link_url = models.CharField(max_length=255, verbose_name="链接地址", null=True, blank=True, help_text="链接地址")
    is_catalog = models.BooleanField(default=False, verbose_name="是否目录", help_text="是否目录", null=True, blank=True)
    ...
    component_name = models.CharField(max_length=50, verbose_name="组件名称", null=True, blank=True,
                                      help_text="组件名称")
    status = models.BooleanField(default=True, verbose_name="菜单状态", help_text="菜单状态", null=True, blank=True)
    cache = models.BooleanField(default=False, verbose_name="是否页面缓存", help_text="是否页面缓存", null=True, blank=True)
    visible = models.BooleanField(default=True, verbose_name="侧边栏中是否显示",
                                  help_text="侧边栏中是否显示", null=True, blank=True)
    is_iframe = models.BooleanField(default=False, verbose_name="框架外显示", help_text="框架外显示", null=True, blank=True)
    is_affix = models.BooleanField(default=False, verbose_name="是否固定", help_text="是否固定", null=True, blank=True)
...
class MenuField(CoreModel):
    ...
    menu = models.ForeignKey(to='Menu', on_delete=models.CASCADE, verbose_name='菜单', db_constraint=False, null=True, blank=True)
    field_name = models.CharField(max_length=64, verbose_name='模型表字段名', null=True, blank=True)
    title = models.CharField(max_length=64, verbose_name='字段显示名', null=True, blank=True)
    class Meta:
...
class FieldPermission(CoreModel):
    role = models.ForeignKey(to='Role', on_delete=models.CASCADE, verbose_name='角色', db_constraint=False, null=True, blank=True)
    field = models.ForeignKey(to='MenuField', on_delete=models.CASCADE,related_name='menu_field', verbose_name='字段', db_constraint=False, null=True, blank=True)
    is_query = models.BooleanField(default=1, verbose_name='是否可查询', null=True, blank=True)
    is_create = models.BooleanField(default=1, verbose_name='是否可创建', null=True, blank=True)
    is_update = models.BooleanField(default=1, verbose_name='是否可更新', null=True, blank=True)
...
class MenuButton(CoreModel):
    ...
    name = models.CharField(max_length=64, verbose_name="名称", help_text="名称", null=True, blank=True)
    value = models.CharField(unique=True, max_length=64, verbose_name="权限值", help_text="权限值", null=True, blank=True)
    api = models.CharField(max_length=200, verbose_name="接口地址", help_text="接口地址", null=True, blank=True)
    METHOD_CHOICES = (
        (0, "GET"),
        (1, "POST"),
        (2, "PUT"),
...
class Dictionary(CoreModel):
    ...
    type = models.IntegerField(choices=TYPE_LIST, default=0, verbose_name="数据值类型", help_text="数据值类型", null=True, blank=True)
    ...
    is_value = models.BooleanField(default=False, verbose_name="是否为value值",
                                   help_text="是否为value值,用来做具体值存放", null=True, blank=True)
    status = models.BooleanField(default=True, verbose_name="状态", help_text="状态", null=True, blank=True)
    sort = models.IntegerField(default=1, verbose_name="显示排序", null=True, blank=True, help_text="显示排序")
...
class OperationLog(CoreModel):
    ...
    status = models.BooleanField(default=False, verbose_name="响应状态", help_text="响应状态", null=True, blank=True)
...
class FileList(CoreModel):
    ...
    file_url = models.CharField(max_length=255, verbose_name="文件地址", help_text="文件地址", null=True, blank=True)
    engine = models.CharField(max_length=100, default='local', verbose_name="引擎", help_text="引擎", null=True, blank=True)
    mime_type = models.CharField(max_length=100, verbose_name="Mime类型", help_text="Mime类型", null=True, blank=True)
    size = models.CharField(max_length=36, verbose_name="文件大小", help_text="文件大小", null=True, blank=True)
    md5sum = models.CharField(max_length=36, verbose_name="文件md5", help_text="文件md5", null=True, blank=True)
...
class Area(CoreModel):
    name = models.CharField(max_length=100, verbose_name="名称", help_text="名称", null=True, blank=True)
    code = models.CharField(max_length=20, verbose_name="地区编码", help_text="地区编码", unique=True, db_index=True, null=True, blank=True)
    level = models.BigIntegerField(verbose_name="地区层级(1省份 2城市 3区县 4乡级)",
                                   help_text="地区层级(1省份 2城市 3区县 4乡级)", null=True, blank=True)
    pinyin = models.CharField(max_length=255, verbose_name="拼音", help_text="拼音", null=True, blank=True)
    initials = models.CharField(max_length=20, verbose_name="首字母", help_text="首字母", null=True, blank=True)
    enable = models.BooleanField(default=True, verbose_name="是否启用", help_text="是否启用", null=True, blank=True)
...
class ApiWhiteList(CoreModel):
    url = models.CharField(max_length=200, help_text="url地址", verbose_name="url", null=True, blank=True)
    METHOD_CHOICES = (
        (0, "GET"),
        (1, "POST"),
        (2, "PUT"),
...
class SystemConfig(CoreModel):
    ...
    title = models.CharField(max_length=50, verbose_name="标题", help_text="标题", null=True, blank=True)
    key = models.CharField(max_length=100, verbose_name="键", help_text="键", db_index=True, null=True, blank=True)
    ...
    status = models.BooleanField(default=True, verbose_name="启用状态", help_text="启用状态", null=True, blank=True)
    ...
    form_item_type = models.IntegerField(
        choices=FORM_ITEM_TYPE_LIST, verbose_name="表单类型", help_text="表单类型", default=0, null=True, blank=True
    )
...
class LoginLog(CoreModel):
    ...
    login_type = models.IntegerField(default=1, choices=LOGIN_TYPE_CHOICES, verbose_name="登录类型",
                                     help_text="登录类型", null=True, blank=True)
...
class MessageCenter(CoreModel):
    title = models.CharField(max_length=100, verbose_name="标题", help_text="标题", null=True, blank=True)
    content = models.TextField(verbose_name="内容", help_text="内容", null=True, blank=True)
    target_type = models.IntegerField(default=0, verbose_name="目标类型", help_text="目标类型", null=True, blank=True)
...
class DownloadCenter(CoreModel):
    ...
    task_name = models.CharField(max_length=255, verbose_name="任务名称", help_text="任务名称", null=True, blank=True)
    task_status = models.SmallIntegerField(default=0, choices=TASK_STATUS_CHOICES, verbose_name='是否可下载', help_text='是否可下载', null=True, blank=True)
    ...
    size = models.BigIntegerField(default=0, verbose_name="文件大小", help_text="文件大小", null=True, blank=True)
