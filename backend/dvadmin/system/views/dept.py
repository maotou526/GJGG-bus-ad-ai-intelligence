...
class DeptViewSet(CustomModelViewSet):
    ...
    filter_fields = ["name", "id", "parent", "company_belong_id"]
    search_fields = []
    # extra_filter_class = []
    ...
    def all_dept(self, request, *args, **kwargs):
        ...
        请求参数:
            company_belong_id: 公司ID（可选），传入则只返回该公司的部门，不传则返回所有部门
        
        返回:
            部门列表，包含 name, id, parent 字段
        """
        queryset = self.filter_queryset(self.get_queryset())
        queryset = queryset.filter(status=True)
        
        # 根据公司ID过滤部门
        company_belong_id = request.query_params.get('company_belong_id', None)
        if company_belong_id:
            queryset = queryset.filter(company_belong_id=company_belong_id)
        
        data = queryset.order_by('sort').values('name', 'id', 'parent', 'company_belong_id')
