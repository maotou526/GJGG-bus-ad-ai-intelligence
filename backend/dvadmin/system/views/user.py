...
class UserViewSet(CustomModelViewSet):
    ...
    filter_fields = ["name", "username", "gender", "is_active", "dept", "user_type", "company_belong_id", "agent_id"]
    ...
    def user_info(self, request):
        ...
        # 代理商信息
        result['agent_id'] = user.agent_id
        result['agent_name'] = None
        if user.agent_id:
            agent = AgentModel.objects.filter(id=user.agent_id).first()
            if agent:
                result['agent_name'] = agent.company_name
        # 营运公司信息
        result['company_id'] = user.company_belong_id
        result['company_name'] = None
        result['company_code'] = None
        if user.company_belong_id:
            company = CompanyModel.objects.filter(id=user.company_belong_id).first()
            if company:
                result['company_name'] = company.fullname or company.shortname
                result['company_code'] = company.encode
    ...
    def list(self, request, *args, **kwargs):
        ...
        company_belong_id = request.query_params.get('company_belong_id')
        ...
        if int(show_all):
            ...
            if dept_id != '':
                ...
                # Manual filtering for company_belong_id when show_all=1
                if company_belong_id:
                    queryset = queryset.filter(company_belong_id=company_belong_id)
            else:
