...
# 配置 Celery 使用 Django 的日志系统
app.conf.worker_hijack_root_logger = False  # 不劫持根 logger，使用 Django 的日志配置
app.conf.worker_log_format = '[%(asctime)s][%(name)s.%(funcName)s():%(lineno)d] [%(levelname)s] %(message)s'
app.conf.worker_task_log_format = '[%(asctime)s][%(name)s.%(funcName)s():%(lineno)d] [%(levelname)s] %(message)s'

# 确保 Celery 使用 Django 的 LOGGING 配置
