...
class BookingOrderModelSerializer(CustomModelSerializer):
    ...
    def validate_booking_type(self, value):
        ...
        if value not in [1, 2, 3]:
            raise serializers.ValidationError("预订单类型只能是：1=新预订, 2=续期, 3=变更")
        return value

    def validate_booking_status(self, value):
        """验证预订单状态"""
        if value not in [1, 2, 3, 4, 5, 6, 7]:
            raise serializers.ValidationError("预订单状态只能是：1=草稿, 2=待审批, 3=审批中, 4=已通过, 5=已完成, 6=已取消, 7=已驳回")
        return value
    ...
    def validate(self, attrs):
        ...
        if booking_type in [2, 3] and not original_booking_id:
